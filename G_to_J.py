...
# @Date:   31-Jul-2020 14:07:75:752  GMT-0600
# @Email:  rijshouray@gmail.com
# @Filename: G_to_J.py
# @Last modified by:   Ray
# @Last modified time: 23-Feb-2021 15:02:12:129  GMT-0700
# @License: [Private IP]
...
def graph_to_json(edge_class, strat_class, top_CONST, VIS_parameters, shape_name, dict_stratClass_edgeClass,
                  d_keys_ind, d_values_ind, edge_weights_3t, color_strat_dict, VIS_location='', init_strat_name=''):
    ...
    if(VIS_parameters[10]):
        ...
        HTML_FILE += str(init_strat_name) + str(edge_class).replace('_', ' ').upper() + " with " + \
            str(strat_class).replace('_', ' ').upper() + "; Top " + str(top_CONST) + " Links </h2>"
    if(VIS_parameters[2]):
        HTML_FILE += """
            <div class='my-legend'>
        ...
        for num in range(len(color_strat_dict)):
            HTML_FILE += """<li><span style='background:""" + list(color_strat_dict.values())[num] + """;'></span>""" + list(color_strat_dict.keys())[num] + """</li>
                        """
        HTML_FILE += """
                </ul>
            </div>
    ...
    for node_num in range(len(dict_stratClass_edgeClass)):
        ...
        color_faded = str(list(color_strat_dict.values())[list(
            color_strat_dict.keys()).index(d_values_ind[node_num])] + 'F2')
        HTML_FILE += """, color: {'border':'""" + color_faded + """', 'highlight':'""" + \
            color_faded + "', " + "'hover':'" + color_faded + "'" + """}"""
...
# This function `list_clustering_graph_preproccessing` is NOT functional, in development
def list_clustering_graph_preproccessing(twod_list, K, top_CONST=0):
    test = [nx.complete_graph(palette) for palette in ALL_RGB_annotations]
    ...
    color_map = ['#e6194b', '#3cb44b', '#ffe119', '#4363d8', '#f58231', '#911eb4', '#46f0f0', '#f032e6', '#bcf60c',
                 '#fabebe', '#008080', '#e6beff', '#9a6324', '#fffac8', '#800000', '#aaffc3', '#808000', '#ffd8b1',
                 '#000075', '#808080', '#ffffff', '#000000'][-K:]
...
def graph_preproccessing(df, edge_class, strat_class, initial_stratifier_name, top_CONST=0, return_level='minimal'):
    ...
    df__dict_stratClass_edgeClass = (df[[edge_class] + [strat_class]]
    ...
    sns.set_palette('hls', n_colors=N)
    palette = sns.color_palette(n_colors=N)
    color_map = ['#%02x%02x%02x' % (int(proper_round(i * 255)), int(proper_round(j * 255)),
                                    int(proper_round(k * 255))) for i, j, k in palette]
    color_strat_dict = dict(zip(list(set(d_values_ind)), color_map))
...
# Undirected --> upper triangle of matrix
# Directed --> nodeA1, nodeB2 (nearest neighbors), node attributes, edge attributes
