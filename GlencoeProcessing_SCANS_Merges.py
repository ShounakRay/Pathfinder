...
# @Last modified time: 24-Feb-2021 16:02:91:914  GMT-0700
# @License: [Private IP]
...
lmids = pd.DataFrame(lettered_mids)
lmids['Type'] = lmids[0].apply(lambda x: re.sub(r'[0-9]', '', x).replace('-', '').strip())
lmids['Dependent'] = lmids[0].apply(lambda x: True if('-' in x) else False)
